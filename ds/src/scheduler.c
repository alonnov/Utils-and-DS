...
#include "task.h"		/* TaskClean, TaskCreate, TaskGetUnID, TaskGetTime, 
						   TaskRun */
#include "uid.h"
#include "pqueue.h" 	/* pqueue_t, PQueueCreate, PQueueDequeue, 
						   PQueueDestroy, PQueueEnqueue, */
...
{
	return TaskGetTime((task_t*)data2) - TaskGetTime((task_t*)data1);
}
...
scheduler_t* SchedulerCreate()
{
	...
	sched->pq_tasks = PQueueCreate(CompFunc);
	if(!sched->pq_tasks)
	{
		free(sched);
		return NULL;
						...
						void* cleanup_params)
{
	...
	if(0 != PQueueEnqueue(sched->pq_tasks, task))
	{
		TaskClean(task);
		return bad_unid;
	}
...
void SchedulerRemoveTask(scheduler_t* sched, unid_t task_id)
{
	...
	task = PQueueRemoveBy(sched->pq_tasks, MatchID, &task_id);
	if(task)
	{
		TaskClean(task);
	}
...
size_t SchedulerRun(scheduler_t* sched)
{
	...
	{
		task = PQueueDequeue(sched->pq_tasks);
		...
		{ 	
			...
			PQueueEnqueue(sched->pq_tasks, task);
		}
		else
		{
			TaskClean(task);	
...
size_t SchedulerSize(const scheduler_t* sched)
{
	...
	return PQueueSize(sched->pq_tasks);
}
...
{	
	...
	{
		TaskClean((task_t*)PQueueDequeue(sched->pq_tasks));
	}
}
...
int SchedulerIsEmpty(const scheduler_t* sched)
{
	...
	return PQueueIsEmpty(sched->pq_tasks);
}
